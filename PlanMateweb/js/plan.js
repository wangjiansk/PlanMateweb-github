; (function ($) {
    ...
    function bindEvent() {
        ...
        $('.nav-link').on('click', function () {
            $('.userSeting').css({ display: 'none' });
            if ($(this).hasClass('open')) {
                $(this).removeClass('open');
                $('.active').removeClass('active');
                $(this).addClass('active');
                if ($(this).find('ul.navChild').length > 0) {
                    $(this).find('ul.navChild').css({ display: 'none' });
                    $(this).children('a').find('i').eq(1).removeClass('fa-angle-down').addClass('fa-angle-right');
                }
            } else {
                $('ul.navChild').css({ display: 'none' });
                $('.open').removeClass('open');
                $(this).addClass('open');
                $('.active').removeClass('active');
                $(this).addClass('active');
                if ($(this).find('ul.navChild').length > 0) {
                    $(this).find('ul.navChild').css({ display: 'block' });
                    $(this).children('a').find('i').eq(1).removeClass('fa-angle-right').addClass('fa-angle-down');
                    $(this).find('ul.navChild').find('li').on('click', function () {
